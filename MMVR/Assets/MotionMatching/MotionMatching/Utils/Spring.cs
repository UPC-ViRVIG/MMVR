...
namespace MotionMatching
{
    ...
    public static class Spring
    {
        ...
        public static void CharacterRotationUpdate(ref quaternion rot, ref float3 angularVelocity, ref float3 angularAcceleration,
                                                   float3 angularVelocityGoal, float halfLife, float deltaTime)
        {
            float3 scaledAxisRot = MathExtensions.QuaternionToScaledAngleAxis(rot);
            float y = HalfLifeToDamping(halfLife) / 2.0f; // this could be precomputed
            float3 j0 = angularVelocity - angularVelocityGoal;
            float3 j1 = angularAcceleration + j0 * y;
            float eyedt = FastNEgeExp(y * deltaTime); // this could be precomputed if several agents use it the same frame

            scaledAxisRot = eyedt * (((-j1) / (y * y)) + ((-j0 - j1 * deltaTime) / y)) +
                            (j1 / (y * y)) + j0 / y + angularVelocityGoal * deltaTime + scaledAxisRot;
            rot = MathExtensions.QuaternionFromScaledAngleAxis(scaledAxisRot);
            angularVelocity = eyedt * (j0 + j1 * deltaTime) + angularVelocityGoal;
            angularAcceleration = eyedt * (angularAcceleration - j1 * y * deltaTime);
        }
