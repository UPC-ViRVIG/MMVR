...
namespace MotionMatching
{
    ...
    public class PoseExtractor
    {
        ...
        public bool Extract(BVHAnimation bvhAnimation, PoseSet poseSet, MotionMatchingData mmData)
        {
            ...
            {
                poses[i] = ExtractPose(bvhAnimation, i, mmData.HipsForwardLocalVector, mmData.ContactVelocityThreshold, poseSet, poses);
            }
        ...
        /// Returns true if the bvhAnimation was added to the poseSet, false otherwise
        /// </summary>
        public bool Extract(BVHAnimation bvhAnimation, PoseSet poseSet, MotionSynthesisData msData)
        {
            // Set Poses
            int nFrames = bvhAnimation.Frames.Length;
            PoseVector[] poses = new PoseVector[nFrames];
            for (int i = 0; i < nFrames; i++)
            {
                poses[i] = ExtractPose(bvhAnimation, i, msData.HipsForwardLocalVector, 0.15f, poseSet, poses);
            }
            //if (msData.SmoothSimulationBone)
            //{
            //    SmoothSimulationBone(poses, poseSet);
            //}
            return poseSet.AddClip(poses, bvhAnimation.FrameTime);
        ...
        private void SmoothContacts(PoseVector[] poses)
        {
            ...
            for (int i = 0; i < poses.Length; i++)
            {
                ...
                bool[] leftFootContactWindow = new bool[windowsRadius * 2 + 1];
                bool[] rightFootContactWindow = new bool[windowsRadius * 2 + 1];
                int windowIndex = 0;
        ...
        private PoseVector ExtractPose(BVHAnimation bvhAnimation, int frameIndex, float3 hipsLocalForward, float contactVelocityTh, PoseSet poseSet, PoseVector[] poses)
        {
            ...
            var jobSB = new AddSimulationBoneBurst
            {
                ...
                HipsForwardLocalVector = hipsLocalForward,
                // Output
            ...
            bool _contactLeftFoot = math.length(jointVelocities[leftToesIndex]) < contactVelocityTh;
            bool _contactRightFoot = math.length(jointVelocities[rightToesIndex]) < contactVelocityTh;
