...
namespace MotionMatching
{
    ...
    public class BVHAnimation
    {
        ...
        // HARDCODED: Research testing :D should do an abstract class or something...
        public void UpdateMecanimInformation(MotionSynthesisData data)
        {
            for (int i = 0; i < Skeleton.Joints.Count; i++)
            {
                Joint joint = Skeleton.Joints[i];
                if (data.GetMecanimBone(joint.Name, out HumanBodyBones bone))
                {
                    joint.Type = bone;
                    Skeleton.Joints[i] = joint;
                }
